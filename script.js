...
(() => {
  ...
  /* =========================================================
     3. AMBIENT AUDIO — supports optional external file with synth fallback
     ========================================================= */
  let audioCtx, masterGain, analyser, isPlaying = false;
  let osc1, osc2, lfo, lfoGain; // synth nodes (when using synth)
  let audioEl = null; // HTMLAudioElement when using external file
  const EXTERNAL_AUDIO_SRC = 'ambient.mp3'; // change this path to your file if needed
  ...
  function buildMediaGraph(mediaElement) {
    audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    masterGain = audioCtx.createGain();
    masterGain.gain.value = document.getElementById('volumeSlider').value / 100 * 0.18;
  ...
  function startAmbientAudio() {
    ...
    try {
      ...
      if (!audioEl && EXTERNAL_AUDIO_SRC) {
        audioEl = new Audio(EXTERNAL_AUDIO_SRC);
        audioEl.loop = true;
      ...
      if (audioEl) {
        try {
          if (!audioCtx) buildMediaGraph(audioEl);
          audioCtx.resume();
          // attempt to play; if browser blocks autoplay, fallback armGestureStart
          audioEl.play().then(() => {
            isPlaying = true;
            updatePlayIcon();
            drawVisualizer();
            setAudioStatus('Music playing');
    ...
    } catch (e) { setAudioStatus('Music unavailable'); }
  }

  function armGestureStart() {
    setAudioStatus('Tap anywhere to start music');
    const start = () => {
      if (!audioCtx) {
        if (audioEl) buildMediaGraph(audioEl);
        else buildSynthGraph();
      }
      audioCtx.resume().then(() => {
        if (audioEl) audioEl.play().catch(() => {});
        isPlaying = true; updatePlayIcon(); drawVisualizer();
        setAudioStatus('Music playing');
      });
  ...
  document.getElementById('audioToggle').addEventListener('click', () => {
    ...
    if (isPlaying) {
      ...
      if (audioEl) {
        audioEl.play().then(() => {
          isPlaying = true; updatePlayIcon(); drawVisualizer(); setAudioStatus('Music playing');
        }).catch(() => {
          setAudioStatus('Tap anywhere to start music');
        });
      } else if (audioCtx) {
  ...
  document.getElementById('volumeSlider').addEventListener('input', e => {
    const v = (e.target.value / 100) * 0.18;
